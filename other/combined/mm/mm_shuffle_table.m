...
for exp_i = 1 : length(experiment)
    ...
    for probe_i = 1 : length(probe_fnames)
        ...
        str_probe = sprintf('Probe %i of %i', probe_i, length(probe_fnames));
        fprintf(str_probe);
        ...
        for trial_i = 1 : length(trials)
            ...
            str_trial = sprintf('Trial %i of %i', trial_i, length(trials));
            fprintf(str_trial);
            ...
            treadmill_motion_mask = trials(trial_i).treadmill_motion_mask();
            analysis_window = trials(trial_i).analysis_window();
%             camera_motion_mask = trials(trial_i).camera_motion_mask;
%             baseline_mask = trials(trial_i).baseline_mask();
            
            %stationary_mask = (analysis_window & ~treadmill_motion_mask & ~camera_motion_mask) | (baseline_mask & ~camera_motion_mask);
...
save('mismatch_nov20_shuffle_table.mat', 'T');
