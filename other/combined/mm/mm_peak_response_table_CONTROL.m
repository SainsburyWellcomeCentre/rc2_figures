...
common_time_base        = linspace(peak_window_t(1), ...
                                   peak_window_t(2), ...
                                   round(range(peak_window_t)*10e3));

                           
% save
save_matfname       = sprintf('mismatch_nov20_%ims_peak_response_CONTROL.mat', avg_window_t*1e3);
...
for exp_i = 1 : length(experiment)
    ...
    for probe_i = 1 : length(probe_fnames)
        ...
        for trial_i = 1 : n_trials
            ...
            % if mm_start_t = 0
            %   avg_windot_t = 100 ms
            %   peak_window_t = [50, 400] ms
            % then we want to start at -100 - 400 - 50 = -550 to set the
            %   search window at -550 to -150
            response_window_time_base = mm_start_t(trial_i) - avg_window_t - peak_window_t(2) - peak_window_t(1) + common_time_base;
        ...
        for prot_i = 1 : n_protocols
            ...
            for cluster_i = 1 : n_clusters
                ...
                [~, response_max_idx] = max(mean(fr_conv_cat, 2));
                [~, response_min_idx] = min(mean(fr_conv_cat, 2));
        ...
        for trial_i = 1 : n_trials
            ...
            for cluster_i = 1 : n_clusters
                ...
                % firing rate
                response_max_window = mm_start_t(trial_i) + response_max_t{probe_i}(prot_idx, cluster_i) + [-avg_window_t, avg_window_t]/2 ...
                    - avg_window_t - peak_window_t(2) - peak_window_t(1);
                response_min_window = mm_start_t(trial_i) + response_min_t{probe_i}(prot_idx, cluster_i) + [-avg_window_t, avg_window_t]/2 ...
                    - avg_window_t - peak_window_t(2) - peak_window_t(1);
