function figure_s5cd(data, h_ax_main, h_ax_histogram)
...
for ii = 1 : size(probe_ids, 2)
    for jj = 1 : size(probe_ids, 1)
        ...
        tuning_plot.plot(tuning{jj, ii});
        
        text(h_ax_main{jj, ii}, 0, yl{jj, ii}(2), txt{jj, ii}, ...
                'fontsize', 6, ...
                'color', cols{jj, ii}, ...
        ...
        if jj == 2 && ii == 1
            ylabel(h_ax_main{jj, ii}, 'FR (Hz)', 'fontsize', 8);
        end
        
        if jj == 3
            xlabel(h_ax_main{jj, ii}, 'T (cm/s)', 'fontsize', 8);
        end
        ...
        shuffle_hist.plot(tuning{jj, ii});
