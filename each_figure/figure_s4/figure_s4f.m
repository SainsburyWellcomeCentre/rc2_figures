...
set(h_ax, 'xscale', 'log', 'xlim', [1e-5, 1], 'ylim', [-0.2, 0.2], 'ytick', [-0.2, 0, 0.2], 'yticklabel', {'-0.2', '0.0', '0.2'});
line(h_ax, [0.05, 0.05], [-0.2, 0.2], 'color', [0.6, 0.6, 0.6], 'linestyle', '--');
line(h_ax, [1e-5, 1], [0, 0], 'color', [0.6, 0.6, 0.6], 'linestyle', '--');

xl = get(h_ax, 'xlim');

text(h_ax, 10^mean(log10(xl)), 0.2, sprintf('Suppressed %.1f%%', 100*sum(~isnan(classification))/length(classification)), ...
