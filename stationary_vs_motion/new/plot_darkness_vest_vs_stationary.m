...
save_dir = 'C:\Users\Lee\Desktop\Desktop\unity_plots\darkness\vestibular';
save_on = true;
...
FigureTitle(gcf, 'All mice pooled. Vest + Visual Flow. Motion vs. Stationary. Darkness');
...
for probe_i = 1 : length(probe_fname)
    ...
    FigureTitle(gcf, sprintf('%s, Vest. Motion vs. Stationary. Darkness', probe_fname{probe_i}));
    
    % print the PDF
    if save_on
        print(fullfile(save_dir, sprintf('%s_vestibular_motion_vs_stationary.pdf', probe_fname{probe_i})), '-bestfit', '-dpdf')
    end
