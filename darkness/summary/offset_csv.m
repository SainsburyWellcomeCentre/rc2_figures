...
figs.save_on = true;
figs.set_figure_subdir('darkness', 'matched_trials_overlay');
...
for probe_i = 1 : length(probe_fnames)
    ...
    for type_i = 1 : length(trial_types)
        ...
        these_trials = dark.trials_of_type(trial_types{type_i});
        
        matched_trial_n = 0;
        
        for trial_i = 1 : length(these_trials)
            
            repeats = dark.get_replay_of(these_trials(trial_i));
            ...
            for rep_i = 1 : length(repeats)
                ...
                offset = dark.match_replay_to_original(these_trials(trial_i), repeats(rep_i));
                ...
                subplot(10, 1, sp_n);
                hold on;
                plot(these_trials(trial_i).filtered_teensy);
                plot(repeats(rep_i).stage(offset:end));
