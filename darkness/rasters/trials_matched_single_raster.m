...
figs.save_on = true;
figs.set_figure_subdir(experiment, 'all_trials_with_cluster_raster');
    ...
    for type_i = 1 : length(trial_types)
        ...
        for trial_i = 1 : length(these_trials)
            ...
            if strcmp(experiment, 'darkness')
                V_trace{type_i}{trial_i} = zeros(length(these_trials(trial_i).filtered_teensy), 1);
            else
            ...
            for rep_i = 1 : length(repeats)
                ...
                %
                offset = exp_obj.get_offset(these_trials(trial_i), repeats(rep_i));
                ...
                if strcmp(experiment, 'darkness')
                    V_trace_rep{type_i}{trial_i}{rep_i} = zeros(length(offset+(0:n_to_plot-1)), 1);
                else
    ...
    for type_i = 1 : length(base_t)
        ...
        for cluster_i = 1 : length(clusters)
            ...
            for trial_i = 1 : length(base_t{type_i})
                ...
                if isempty(base_t{type_i}{trial_i})
                    continue
                end
