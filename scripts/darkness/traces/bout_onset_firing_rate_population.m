...
for prot_i = 1 : length(trial_types)
    ...
    for resp_i = 1 : 3
        mean_fr{resp_i} = mean(mean_fr_each_cluster(:, cat_response_type == resp_type(resp_i)), 2);
        sem_fr{resp_i} = std(mean_fr_each_cluster(:, cat_response_type == resp_type(resp_i)), [], 2)/sqrt(size(mean_fr_each_cluster(:, cat_response_type == resp_type(resp_i)), 2));
    end
    ...
    for resp_i = 1 : 3
        ...
        mean_fr_sub = mean_fr{resp_i}(1:4:end);
        sem_fr_sub = sem_fr{resp_i}(1:4:end);
        h = fill([common_t_sub, common_t_sub(end:-1:1)], ...
            [mean_fr_sub - sem_fr_sub; mean_fr_sub(end:-1:1) + sem_fr_sub(end:-1:1)]', ...
            resp_cols{resp_i}, 'facealpha', 0.2);
        set(h, 'edgecolor', 'none');
        plot(common_t_sub, mean_fr_sub, 'color', resp_cols{resp_i}, 'linewidth', 2);
    end
    ...
    set(h_ax(prot_i, 2), 'plotboxaspectratio', [3, 1, 1]);
...
for prot_i = 1 : length(trial_types)
    ...
    set(h_ax(prot_i, 1), 'ylim', [-2, 30]);
    set(h_ax(prot_i, 2), 'ylim', yL);
    
    set(h_ax(prot_i, 1), 'xlim', [prepad, postpad]);
    set(h_ax(prot_i, 2), 'xlim', [prepad, postpad]);
